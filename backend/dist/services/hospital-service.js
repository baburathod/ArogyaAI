...
export function findNearbyHospitals(latitude, longitude, maxDistanceKm = 15, searchQuery) {
    ...
    if (searchQuery) {
        ...
        results = results.filter((hospital) => hospital.name.toLowerCase().includes(normalized) || hospital.address.toLowerCase().includes(normalized));
    }
 ...
 */
function parseExternalApiResponse(data) {
    if (!data)
        return [];
    ...
    if (Array.isArray(data)) {
        ...
        })).filter((h) => h.name && h.latitude && h.longitude);
    }
...
export async function externalHospitalLookup(query, latitude, longitude, maxDistanceKm = 15) {
