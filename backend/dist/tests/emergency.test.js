...
// Mock database
const mockDb = {
    createEmergency: vi.fn(async (data) => ({
        _id: 'emergency-1',
        ...data,
        status: 'active',
        createdAt: new Date(),
    ...
    getEmergencyById: vi.fn(async (id) => ({
        _id: id,
        userId: 'user-1',
        latitude: 12.9716,
        longitude: 77.5946,
    ...
    assignResponderToEmergency: vi.fn(async (emergencyId, responderId) => ({
        _id: emergencyId,
        responderId,
    })),
    updateEmergencyStatus: vi.fn(async (id, status) => ({
        _id: id,
        status,
    })),
};
...
describe('Emergency APIs (Phase 6)', () => {
    ...
    describe('POST /api/emergencies/:id/respond', () => {
        ...
        it('should only allow responder role', async () => {
            const mockReq = { user: { id: 'user-1', role: 'patient' } };
