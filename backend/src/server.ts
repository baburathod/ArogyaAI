...
import { validate, Emergency, HealthRecordSchema, EmergencySchema, AppointmentSchema, NotificationSchema } from './utils/validators';
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('registerResponder', (responderId: string) => {
    if (responderId) {
...
function authenticateToken(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  try {
    const payload = jwt.verify(token, JWT_SECRET) as { sub?: string; id?: string; email?: string; role?: string };
...
function requireResponderOrAdmin(req: AuthenticatedRequest, res: Response, next: NextFunction) {
...
function requireAuthenticated(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  if (!req.user) {
...
app.get('/api/hospitals/nearby', async (req, res) => {
  ...
  const maxDistanceKm = parseFloat((maxDistance as string) || '15');
...
// Accept lightweight offline payloads and persist to server-side queue files for later processing
// ============================================
...
app.post('/api/offline/medications', async (req, res) => {
  // payload: { medId?, name, takenAt, userId? }
  const payload = req.body || {};
  await ensureDataDir();
  const file = path.join(dataDir, 'offline-medications.json');
  try {
    let arr = [];
    try {
    ...
    await dbUtils.enqueueOfflinePayload('medication-taken', payload).catch(() => null);
    return res.status(202).json({ accepted: true });
  } catch (error) {
    return res.status(500).json({ error: 'Failed to persist offline medication' });
  }
});

/**
 * POST /api/medications/taken
 * Endpoint used by frontend offline replay when a medication was taken.
 * Accepts payload: { medId?, name, takenAt, userId? }
 */
app.post('/api/medications/taken', async (req, res) => {
  ...
  try {
    ...
    const entry = { payload, receivedAt: new Date().toISOString(), source: 'medication-taken' };
    arr.push(entry);
    await fs.writeFile(file, JSON.stringify(arr, null, 2), 'utf8');
    await dbUtils.enqueueOfflinePayload('medication-taken', payload).catch(() => null);

    // Optionally create a lightweight notification for the patient
    try {
      if (payload.userId) {
        await dbUtils.createNotification({
          ...
          message: `Marked medication as taken: ${payload.name || payload.medId || 'unknown'}`,
          severity: 'info',
          createdAt: new Date(),
        });
      }
    ...
    return res.status(500).json({ error: 'Failed to persist medication taken' });
  }
});

app.post('/api/offline/appointments', async (req, res) => {
  // payload: { title, when, createdAt, userId? }
  const payload = req.body || {};
  await ensureDataDir();
  const file = path.join(dataDir, 'offline-appointments.json');
  try {
    let arr = [];
    try {
      const raw = await fs.readFile(file, 'utf8');
    ...
    arr.push({ payload, receivedAt: new Date().toISOString() });
    await fs.writeFile(file, JSON.stringify(arr, null, 2), 'utf8');
    await dbUtils.enqueueOfflinePayload('appointment', payload).catch(() => null);
    return res.status(202).json({ accepted: true });
  } catch (error) {
    return res.status(500).json({ error: 'Failed to persist offline appointment' });
  }
});

// ============================================
// ANALYTICS ENDPOINTS (symptoms data for charts)
// ============================================

app.get('/api/analytics/symptoms', async (req, res) => {
  try {
    // In production this would aggregate from health records; here we return sample data
    const sample = [
    ...
    res.status(500).json({ error: 'Failed to fetch symptom analytics' });
  }
});
...
app.post('/api/emergencies/sos', async (req: AuthenticatedRequest, res) => {
  const validation = validate<Emergency>(EmergencySchema, req.body);
  if (!validation.success) return res.status(400).json({ error: validation.error });
...
app.post('/api/emergencies/:id/respond', requireResponderOrAdmin, async (req: AuthenticatedRequest, res) => {
...
app.post('/api/emergencies/:id/assign', requireResponderOrAdmin, async (req: AuthenticatedRequest, res) => {
...
app.get('/api/emergencies/nearby', async (req, res) => {
  ...
  try {
    const emergencies = await dbUtils.findEmergenciesNearby(
      parseFloat(longitude as string),
      parseFloat(latitude as string),
      parseInt((maxDistance as string) || '5')
    );
    res.json(emergencies);
  } catch (error) {
 ...
 */
app.post('/api/ai/analyze-symptoms', async (req: Request, res: Response) => {
...
/**
 * POST /api/ai/body-region-analysis
 * Interactive body map symptom analysis
 */
app.post('/api/ai/body-region-analysis', async (req: Request, res: Response) => {
  const { userId, region, symptoms, duration, severity, medicalHistory, currentMedications, language } = req.body;

  if (!userId || !region || !symptoms) {
    return res.status(400).json({ error: 'userId, region, and symptoms required' });
  }

  try {
    const result = await AIHealthcare.processBodyRegionWithAI({
      userId,
      region,
      symptoms,
      duration,
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Body region analysis failed' });
  }
});
 ...
 */
app.post('/api/ai/detect-emergency', async (req: Request, res: Response) => {
 ...
 */
app.post('/api/ai/medication-recommendations', async (req: Request, res: Response) => {
 ...
 */
app.get('/api/ai/wellness-report/:userId', async (req: Request, res: Response) => {
  try {
 ...
 */
app.get('/api/ai/population-health', async (req: Request, res: Response) => {
  try {
