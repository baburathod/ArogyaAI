 ...
 */
async function testMultiClientFlow(): Promise<void> {
  return new Promise((resolve, reject) => {
 ...
 */
async function testEmergencySosEndpoint(): Promise<void> {
 ...
 */
async function testHospitalLookup(): Promise<void> {
  const response = await fetch(
    `${BACKEND_URL}/api/hospitals/nearby?latitude=12.9716&longitude=77.5946&maxDistance=15`,
    { method: 'GET' }
  );
...
async function testRateLimiting(): Promise<void> {
  ...
  for (let i = 0; i < 5; i++) {
    promises.push(
      fetch(`${BACKEND_URL}/api/hospitals/nearby?latitude=12.9716&longitude=77.5946`)
    );
  }
