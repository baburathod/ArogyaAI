...
HealthRecordSchema.virtual('bmi').get(function () {
  if (!this.data.weight || !this.data.height) return null;
 ...
 */
interface IHealthRecordStatics {
  findByUser(userId: mongoose.Types.ObjectId, limit?: number): Promise<IHealthRecord[]>;
  findLatestVitals(userId: mongoose.Types.ObjectId): Promise<IHealthRecord | null>;
  findByDateRange(userId: mongoose.Types.ObjectId, startDate: Date, endDate: Date): Promise<IHealthRecord[]>;
}

/**
 * Static Methods
 */
HealthRecordSchema.static('findByUser', function (userId: mongoose.Types.ObjectId, limit = 50) {
  return this.find({ userId }).sort({ date: -1 }).limit(limit);
});

HealthRecordSchema.static('findLatestVitals', function (userId: mongoose.Types.ObjectId) {
  return this.findOne({ userId, recordType: 'vitals' }).sort({ date: -1 });
});

HealthRecordSchema.static('findByDateRange', function (
  userId: mongoose.Types.ObjectId,
  startDate: Date,
  endDate: Date
) {
 ...
 * Export Model
 */
export const HealthRecord = mongoose.model<IHealthRecord, mongoose.Model<IHealthRecord> & IHealthRecordStatics>(
  'HealthRecord',
  HealthRecordSchema
);
